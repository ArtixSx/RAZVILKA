...
for(const m of html.matchAll(/<script[^>]+src="([^"]+)"/g))test('current asset cache '+m[1],()=>{assert.match(m[1],/\?v=0\.18\.2-dc1-review1$/);assert.ok(read(root+m[1].split('?')[0].replace(/^\//,'')).length>0);});
