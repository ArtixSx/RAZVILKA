...
var excluded = []netip.Prefix{
	...
	netip.MustParsePrefix("::ffff:0:0/96"), netip.MustParsePrefix("fc00::/7"),
...
// PublicPrefix checks the entire range, not just its first address. A public
// starting address can still cover private, special-use or mapped addresses.
func PublicPrefix(prefix netip.Prefix) bool {
	if !prefix.IsValid() || prefix.Addr().Is4In6() {
		return false
	}
	prefix = prefix.Masked()
	if !PublicAddress(prefix.Addr()) {
		return false
	}
