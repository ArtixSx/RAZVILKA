...
func specifications() []specification {
	return []specification{
		{id: "nfqws2", name: "NFQWS2", kind: "dpi", binaries: []string{"/opt/usr/bin/nfqws2", "/opt/bin/nfqws2", "/usr/bin/nfqws2"}, binaryNames: []string{"nfqws2"}, files: []string{"/opt/etc/nfqws2/nfqws2.conf"}, initScripts: []string{"/opt/etc/init.d/S51nfqws2"}, description: "Локальный обход DPI · без внешнего сервера", capabilities: []string{"dpi-desync", "tcp", "udp", "nfqueue"}, nativeCheck: true, ownership: Ownership{NFQueues: []string{"dynamic:config"}}},
