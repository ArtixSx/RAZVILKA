...
func Specs() []Spec {
	specs := []Spec{
		...
		{SchemaVersion: 1, ID: "usque", Name: "WARP · MASQUE", Category: "tunnel", Package: "usque-keenetic", Provider: "opkg", Repository: "https://side-effect-tm.github.io/usque-keenetic/all", Description: "Cloudflare WARP через MASQUE (QUIC/HTTP2)", UseCase: "Туннель для полных IP-блокировок; основной транспорт использует UDP/443, TCP/443 проверяется как запасной.", Requirement: "Нужен доступ к Cloudflare endpoint; RAZVILKA использует Sing-box как локальный TUN-адаптер.", Recommended: true, Removable: true, Capabilities: []string{"masque", "quic", "http2", "tun", "split-routing"}, Dependencies: []string{"sing-box"}, Claims: []Claim{{Kind: "tun", Value: "managed"}, {Kind: "policy-table", Value: "managed"}}, Budget: ResourceBudget{RAMMiB: 32, FlashMiB: 20, CPUClass: "medium"}},
