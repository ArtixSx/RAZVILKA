...
**`0.18.2-rc.4`**, исправленный DC1; стабильный канал — `v0.18.0`.
...
1. **Приёмка `v0.18.2-rc.4`.** Пройти CI точного commit, четыре сборки,
   установку и перезапуск финального бинарника. На роутере проверить новую
   схему `60–69` с ранним маркированным правилом другого VPN и без него,
   фактический путь свежих соединений, отрицательный контроль клиента вне
   области и сохранность чужих правил. У rc.3 немаркированный поток через TUN
   подтверждён; конфликт маркированного правила этим не закрыт.
   Проверить отказ при чужих/дублирующихся ранних правилах, точную миграцию
   старых собственных координат и откат к записанному снимку.
   Исторический commit без живого собственного runtime не должен давать
   `live_active`. Отдельно повторить восстановление с ожиданием до 9 минут,
   результат `75` при подтверждённой занятости и обычный откат после
   освобождения неработающего маршрута. Итоги — в `VALIDATION_RU.md`
   на странице выпуска; CI и аппаратная готовность rc.4 пока не подтверждены.
