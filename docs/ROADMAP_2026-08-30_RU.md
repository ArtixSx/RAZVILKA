...
**`0.18.2-rc.3`**, исправленный DC1; стабильный канал — `v0.18.0`.
...
1. **Выпуск и установка `v0.18.2-rc.3`.** Проверить точный релизный commit,
   четыре сборки, пакеты и установку/запуск финального бинарника на роутере.
   Повторить сценарий обновления с применённым VLESS: опубликованный rc.2
   откатился из-за проверки до завершения восстановления. rc.3 должен
   дождаться восстановления до 9 минут и пройти строгую приёмку. Если API
   подтверждает занятую приватную операцию по истечении срока, ожидается
   код `75`: процесс сохранён, готовность не подтверждена, отката нет.
   После освобождения операции неработающий маршрут должен вызвать обычный
   откат. Лимит одной операции восстановления — 8 минут.
