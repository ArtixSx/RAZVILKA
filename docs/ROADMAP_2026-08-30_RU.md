...
Обновлено 14 сентября 2026 года. Текущая линия исходников —
**`0.18.2-rc.6`**, исправление панели DC1; стабильный канал — `v0.18.0`.
...
**Продолжение приёмки:** после первоначальных успешных проверок rc.5
обнаружена потеря собственной цепочки `filter` с отказом клиентского доступа.
[Отчёт rc.5](releases/0.18.2-rc.5-validation.md) сохраняет оба результата.
rc.6 добавляет восстановление целиком исчезнувших собственных цепочек;
[описание](releases/0.18.2-rc.6.md). Его полный CI и аппаратная проверка
официального архива требуются отдельно. Ниже результаты rc.5 относятся
только к зафиксированным первоначальным проверкам.


## Подтверждённые этапы и ближайшая очередь

**Приёмка rc.5 выполнена в проверенной области.** Полный CI и четыре сборки
прошли. На Netcraze ARM64 проверены установка официального архива, штатный
перезапуск, вход с сохранённой учётной записью, 16 сервисов и полные формы
NFQWS2 без изменения исходного файла. Список сервисов загружен за 1,39 секунды;
два свежих HTTPS-запроса Telegram прошли через TUN с приростом счётчиков.
[Изменения](releases/0.18.2-rc.5.md) и
[отчёт с ограничениями](releases/0.18.2-rc.5-validation.md).
История rc.4 ниже сохраняется отдельно; новая приёмка не закрывает остальные
аппаратные сценарии.

1. **Приёмка `v0.18.2-rc.4` — выполнена в проверенной области.**
   CI и Release CI точного commit, четыре сборки, обновление через SSH
   и перезапуск приложения с восстановлением VLESS прошли на Netcraze aarch64.
   Свежие обычные соединения и отдельный тестовый поток с пакетной маркой
   прошли через TUN: HTTPS 200, точный путь соединения и прирост RX/TX.
   Временное правило удалено. Для другого адреса LAN выполнен только
   запрос маршрута. [Точный отчёт](releases/0.18.2-rc.4-validation.md).
   Отдельно остаются аппаратные проверки чужих/дублирующихся ранних правил,
   потери runtime, миграции других адаптеров и отката. Все случаи ожидания
   до 9 минут, busy `75` и аварийного освобождения операции на железе этим
   результатом не объявляются пройденными.
