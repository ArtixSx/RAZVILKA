...
Дата пересмотра: 4 сентября 2026 года<br>
...
Read-only API списка уже подготовлен: он не раскрывает приватный материал и не
разрешает выбор непроверенных записей. Следующий блок N1.2 — пользовательская
страница и явные операции импорта/удаления, после них exact-node проверка.
