...
Read-only API и пользовательская страница списка уже подготовлены: они не
раскрывают приватный материал, показывают источник, срок и честный статус
`не проверен`, не разрешают выбор непроверенных записей. Следующий блок N1.2 —
явные операции alias/disable/delete/reveal с отдельным подтверждением и
no-secret snapshot tests; после него — N1.3 exact-node проверка.
