...
После правок блока `6aed8ef` прошли полный `go test ./... -timeout=90s`,
`go vet ./...`, синтаксис `app.js`/`cloudflare-accounts.js`,
`test-probe-ui.mjs`/`test-cloudflare-ui.mjs` и сборка всех Go-пакетов для Linux
...
Новый раздел копий проверен в браузере на изолированном loopback-стенде с
вымышленным профилем: preview → save → list и ширина 390 px. Стенд остановлен,
временная вкладка закрыта. Это не визуальный аудит всех вкладок и не HIL.
...
2. Довести PR-1.1: lifecycle ownership, явная миграция с сохранением исходных
   ...
   внутренний backup/restore и аутентифицированные passive preview/import/list
   API/UI готовы. Неизвестный writer lock не удалять без доказанного владельца;
   до выдачи криптоопераций через API ограничить суммарную параллельность KDF.
