...
Текущая линия — **`0.18.2-rc.3`**, исправленный DC1. Канонический подробный
...
Ближайший сценарий rc.3 — обновление с применённым VLESS: ожидать до 9 минут
перед строгой проверкой. Подтверждённая API занятость восстановления в конце
срока означает код `75` и сохранённый процесс с неподтверждённой готовностью,
без успеха или отката. После освобождения операции неработающий маршрут
ведёт к обычному откату.
У опубликованного rc.2 этот сценарий завершился преждевременным откатом.

Следующие этапы: проверка и установка точного релизного артефакта; аппаратная
...
версии/хэши относятся к своему срезу и не описывают готовность `0.18.2-rc.3`.
