...
> Архивный снимок `v0.15.0` от 24 августа 2026 года. Актуальный уровень
> готовности находится в [CURRENT_STATUS_RU.md](CURRENT_STATUS_RU.md), порядок
> дальнейшей реализации — в [ROADMAP_2026-08-30_RU.md](ROADMAP_2026-08-30_RU.md).
...
## Осознанные ограничения `v0.15.0`
...
- Telegram проверяется по трём независимым HTTPS-сценариям и официальным CIDR,
  но полноценное подтверждение MTProto, media/voice и IPv6 всё ещё требует
  теста с реального LAN-клиента.
- DNS-профили, прямые проверки, обнаружение владельца порта 53 и план
  snapshot/canary/rollback готовы. Live DNS не включён до реализации
  платформенного адаптера Keenetic/Netcraze и аппаратного recovery-теста.
...
До массового релиза должны быть закрыты не только аппаратные тесты, но и P0 из
master plan: единая доказательность, контрольные probes, deadlines/cancel,
конфликты, audit, recovery Safe Mode и canary Apply. DNS, AutoPilot и новые
провайдеры могут выпускаться последующими minor-этапами, но не должны обходить
этот контракт безопасности.
...
До прохождения этой матрицы проект остаётся pre-1.0. Малые совместимые
исправления повышают patch, крупная функция или переработка формата — minor;
`1.0.0` не назначается по одному успешному запуску.
