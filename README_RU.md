...
- сравнивает маршруты изолированными проверками, измеряет TTFB и целостность первых 32 КБ ответа и использует только подтверждённые результаты для `AUTO`;
...
- заранее показывает наличие NFQUEUE, TProxy, socket match, ipset и conntrack для совместимых TCP/UDP/QUIC-схем;
