...
- обязательные сценарии сервиса: Telegram отдельно проверяет сайт, Web-клиент и Core/API; отказ одного обязательного сценария не скрывается общим успехом;
- подбор стратегий обычного NFQWS2 в Strategy Lab без установки z2k как второго сервиса;
- отдельная вкладка DNS с профилями Cloudflare, Quad9, AdGuard и Google, прямой проверкой серверов и безопасным планом; live-DNS пока закрыт до аппаратной проверки интеграции;
...
Архитектура описана в [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), актуальный план — в [docs/MASTER_PLAN_RU.md](docs/MASTER_PLAN_RU.md), критерии массового выпуска — в [docs/PRODUCT_GAPS_RU.md](docs/PRODUCT_GAPS_RU.md).
