...
## В разработке — единый план

- Исправлено определение MIPS/MIPSel в Entware с компактным BusyBox; добавлен понятный пример ручного выбора архитектуры.
- Проверка резервных узлов доступна при остановленных маршрутах; просроченные подключения больше не возвращаются в выбор после смены сети.
- Добавлен просмотр сохранённых сервисов и устройств во время долгой проверки.
- Показаны возраст данных и отсутствие свежего результата; после операции управление восстанавливается.
- Исправлены ложный рабочий режим при недоступном состоянии и очистка сохранённого представления после выхода.

## 0.18.10

- «Открыть изменения» показывает изменённые сервисы и текущий/выбранный маршрут.
- Кнопки применения и отмены доступны без раскрытия подробных настроек.
- Действующие сервисы остаются в «Моих сервисах», даже если ожидается их выключение.
- Исправлены переходы к изменениям устройств, DNS и списков доменов.

## 0.18.9

- Исправлено получение состояния проекта после входа через сохранённую сессию; добавлен «Лог» с причинами ошибок.
- Устранено преждевременное завершение проверки работающих маршрутов на роутере; таймаут и другие причины неподтверждённого состояния объясняются в логе.
- В карточки обходов добавлены версии и установка, обновление, удаление с проверкой зависимостей.
- Исправлены обновление HTTPS-каталогов Entware, получение сетей Telegram и обработка отказов источников.
- Автопилот проверяет следующий резерв после подтверждённого отказа кандидата; общий сбой и ошибка применения останавливают раунд.
- Обновлены главная страница GitHub, инструкции и навигация; добавлен план развития автоматики.
