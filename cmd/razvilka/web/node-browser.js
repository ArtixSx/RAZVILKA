...
function nodeBrowserActive() {
  return !nodeBrowser.authRequired && (nodeBrowser.viewActive ?? state.currentView === 'nodes') && $('#authScreen').hidden && !document.hidden;
}
...
function renderNodeBrowser() {
  ...
  $('#nodeSubscriptions').hidden = nodeBrowser.tab !== 'subscriptions';
  $('#nodePublicCatalogs').hidden = nodeBrowser.tab !== 'public';
...
function bindNodeBrowser() {
  ...
  document.addEventListener('razvilka:view-change', event => { renderWorkspaceNavigation(event.detail); nodeBrowser.viewActive = event.detail === 'nodes'; if (nodeBrowser.viewActive) pollNodeBrowserChecks(); else stopNodeBrowserRefresh(); });
