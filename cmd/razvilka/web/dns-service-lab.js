...
function resultMarkup(result){
 ...
 const statuses={resolved:'Адрес получен',error:'Запрос не завершён',rejected:'Ответ отклонён','no-address':'Нет адреса этой семьи',nxdomain:'Домен не существует по ответу DNS',unknown:'Нет результата'};
 ...
 return '<div class="dc1-result-note">Это DNS-ответы, не подтверждение работы сервиса на ваших устройствах.</div>'+result.results.map(row=>`<div class="dc1-answer"><div><strong>${escape(row.provider_id)}</strong><small>${escape(row.family)} · DoH</small></div><div><b>${escape(statuses[row.status]||'Неизвестный статус')}</b><small>${escape((row.addresses||[]).slice(0,8).join(', ')||errors[row.error_code]||row.error_code||'—')}</small>${Number.isInteger(row.ttl_seconds)?`<small>Срок DNS-ответа: ${escape(row.ttl_seconds)} с</small>`:''}${['nodata','nxdomain'].includes(row.negative_kind)&&Number.isInteger(row.negative_ttl_seconds)?`<small>Срок отрицательного ответа: ${escape(row.negative_ttl_seconds)} с</small>`:''}</div></div>`).join('');
}
