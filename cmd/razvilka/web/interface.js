...
function renderInterfaceHome(summaries){
 ...
 const title=!ready?'Состояние ещё не получено':stopped?'Маршруты RAZVILKA остановлены':!a.selected?home.appliedCount?'Есть действующие маршруты':'Добавьте первый сервис':a.failed?'Есть неуспешные проверки':a.pending?'Нужны свежие подтверждения':'Последние проверки пройдены';
 ...
 const firstRun=ready&&!a.selected&&!home.appliedCount&&!stopped;
 $('#ui3HealthBanner').dataset.state=tone;
 const healthDetails=firstRun?`<div class="home-start-steps"><div><span>01</span><b>Выберите сервисы</b><small>Сайты и приложения из каталога или ваши собственные</small></div><div><span>02</span><b>Настройте Автопилот</b><small>Разрешите подходящие обходы и автоматические проверки</small></div></div><button type="button" class="primary" data-console-navigate="onboard" data-rz-focus="home-start">${ci('route')} Начать настройку ${ci('chevron')}</button>`:`<div class="r42-health-number"><strong>${ready?a.good:'—'}</strong><span>/ ${ready?a.selected:'—'}<small>сервисов прошли последнюю проверку</small></span></div><div class="r42-health-bar" role="img" aria-label="${a.good} подтверждено, ${a.pending} без подтверждения, ${a.failed} не прошли проверку">${bars}</div><div class="r42-health-legend">${ready?`<span><i class="good"></i>${a.good} подтверждено</span><span><i class="unknown"></i>${a.pending} без подтверждения</span><span><i class="bad"></i>${a.failed} не прошли</span>`:'<span>Нет данных о сервисных проверках</span>'}</div>`;
 interfaceHTML('ui3HealthBanner',`<div class="r42-panel-cap"><span class="ui3-eyebrow">${firstRun?'ПЕРВОЕ ПОДКЛЮЧЕНИЕ':'ДОСТУПНОСТЬ СЕРВИСОВ'}</span><span class="ui3-status ${tone}">${ci(tone==='good'?'check':'shield')}${ready?'Локальная панель':'Ожидаем данные'}</span></div><h3>${esc(title)}</h3><p>${esc(desc)}</p>${healthDetails}`);
